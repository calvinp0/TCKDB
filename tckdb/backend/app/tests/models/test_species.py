...
def test_species_model():
    ...
    assert species_1.hessian == [[1, 2, 3], [2, 5, 4], [3, 4, 9]]
    assert species_1.frequencies == [132.2, 548.5, 1032.5, 2015.22, 2018.12, 3005.22]
    assert species_1.scaled_projected_frequencies == [
        130.217,
        540.2725,
        1017.013,
        1984.992,
    ...
    assert species_1.normal_displacement_modes == [
        [0.125, 0.89, 0.35],
        [-0.25, 0.25, -0.89],
        [0.56, 0.98, -0.65],
        [0.022, -0.005, 0.5],
    ...
    assert species_1.heat_capacity_model == {
        "model": "NASA",
        "T min": 100,
        "T max": 5000,
        "coefficients": {
...
def test_species_relationships():
    ...
    assert (
        str(species_1.opt_level) == "wB97xd/def2TZVP UltraFine solvation: PCM in water"
    )
    ...
    ess_2 = ESS(
        name="Gaussian", version="16", revision="C.01", url="https://gaussian.com/"
    )
